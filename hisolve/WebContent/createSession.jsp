...
<body>
	...
	<script>
		if(sessionStorage.getItem("allowed")!="professorPage.jsp"){
			alert("잘못된 접근입니다.");
			history.back();
		}
		else{
			sessionStorage.setItem("allowed", "createSession.jsp");
		}
	</script>
			...
			<form id="postData">
				...
				<div class="input-group mb-3">
  					<div class="input-group-prepend">
    					<label class="input-group-text" for="sessionName">세션 이름:</label>
  					</div>
  					<input name="sessionName" id="sessionName" type="text" class="form-control" required>
...
<script>
	...
	$("#openTime").datetimepicker("setDate",currentDate);
	$("#closeTime").datetimepicker("setDate",currentDate);
	...
	$("#submit").click(function(){
		...
		if($('#sessionName').val()==""){
			...
			$.ajax({
	        	...
	        	dataType:'text',
	        	type:'POST',
	        	data:{'userID':sessionStorage.getItem("userID"), 'sessionName':$('#sessionName').val(),},
	        	success : function(data){
	        		if(parseInt(data)==1){
	        			alert('세션 이름이 중복되었습니다. 다른 이름을 입력해주세요.');
	        		}
	        		else{
	        			$.ajax({
	    		        	url:'uploadSession.jsp',
	    		        	dataType:'text',
	    		        	type:'POST',
	    		        	data:{'userID':sessionStorage.getItem("userID"), 'sessionName':$('#sessionName').val(), 'openTime':$('#openTime').val(), 'closeTime':$('#closeTime').val(),},
	    		        	success : function(){
