<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	if(timeover == 0){
	    ...
	    String command[] = {"/usr/local/bin/docker exec -w /home/ " + container + " mkdir " + userID,
				...
				"/usr/local/bin/docker cp " + txtPath + "/input " + container +":/home/" + userID,
				"mkdir " + uploadPath + "/userAnswer",
	    ...
	    for(int i = 0; i < 9; i++){
	    	try{
	    		if(i == 4){		//컨테이너 내부 프로그램 run할 때 exit 코드를 출력하기 위해 shell 사용
	   			...
	   			if(i == 1 && type.equals("multi")){
	   				ps = rt.exec("/usr/local/bin/docker exec -w /home/" + userID + " "+ container + " tar -xvf " + fileName);
	   				ps.waitFor(); 
	   				ps = rt.exec("/usr/local/bin/docker cp " + uploadPath + "/makefile " + container + ":/home/" + userID);
	   				ps.waitFor();
	   			}
	   			...
	   			if(i == 4){
	   				if((line = br.readLine()) != null){
	   					switch(line){
	   						case "0":	//정상 종료
	   							ps = rt.exec("/usr/local/bin/docker cp " + container +":/home/" + userID + "/userAnswer/userAnswer" + String.valueOf(count) + ".txt "+ uploadPath + "/userAnswer");
	   							ps.waitFor();
	   							ps = rt.exec("diff --strip-trailing-cr " + txtPath + "/output/output" + String.valueOf(count) + ".txt " + uploadPath + "/userAnswer/userAnswer" + String.valueOf(count) + ".txt");
	   							ps.waitFor();
	   							BufferedReader br1 =
	   				    	 			new BufferedReader(
	   				    	 					new InputStreamReader(
	   					...
	   					if(check == 0){
	   						if(count < Integer.valueOf(inputNum.trim())){
		    					i -= 1;
		    					command[4] = "/usr/local/bin/docker exec -w /home/ " + container + " /bin/bash -c 'ulimit -Su 100 ; python container.py " + userID + " " + fileName + " " + String.valueOf(count) + " " + timeLimit + " " + type + "' ; echo $?";
		    				}
	   					}
	   					continue;
	    			}
